...
import { InstallerOptions } from '../helpers/logNextSteps.js';
...
export const scaffoldProject = async ({
  ...
  const srcDir = path.join(PKG_ROOT, 'templates/DgraphDB');
  ...
  if (fs.existsSync(projectDir)) {
    if (fs.readdirSync(projectDir).length === 0) {
      ...
      if (overwriteDir === 'clear') {
        spinner.info(`Emptying ${chalk.cyan.bold(projectName)} and creating Dgraph project..\n`);
